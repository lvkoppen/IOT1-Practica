...
let devices = [
    ...
    { "deviceId" : 4, "room" : "Bathroom", "helderheid" : 255, "verbindingstype" : "iothub"},
...
app.get("/lampen/alarm", (req, res) => {

    devices.forEach((lamp) => {
        if(lamp.verbindingstype == 'iothub'){
            alarm("alarm", lamp.deviceId)
        }
    })

    mqttclient.publish('lampen/alarm', JSON.stringify({message: "alarm!"}))

    return res.json({message: "Alarm melding"})
});
